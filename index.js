...
function PhpSwitchAccessory(log, config) {
  ...
  this.switchName = config["switch_name"] || this.name; // fallback to "name" if you didn't specify an exact "bulb_name"
...
PhpSwitchAccessory.prototype.getPowerOn = function(callback) {
  ...
  request("http://localhost:8000/simple_server.php?action=get", 
...
PhpSwitchAccessory.prototype.setPowerOn = function(powerOn, callback) {
  ...
  // console.log("Requesting: " + "http://localhost:8000/simple_server.php?" + requestContent);
  request("http://localhost:8000/simple_server.php?" + requestContent
    , function (error, response, body) {
      callback(null);
    });
}
